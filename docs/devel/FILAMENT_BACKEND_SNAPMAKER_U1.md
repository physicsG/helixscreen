...
- `NUM_TOOLS = 4`, slot `i` carries `extruder_name` `"extruder"` / `"extruder{i}"`
  (`ams_backend_snapmaker.cpp:278-286`).
- `PathTopology::PARALLEL` on both the unit and `get_topology()`
  (`include/ams_backend_snapmaker.h:131-134`). Because every lane has an independent
  path, `needs_unload_before_load()` is answered by the base class — the serial
  lane rule never applies (`include/ams_backend_snapmaker.h:136-139`).
- `tip_method = TipMethod::NONE` — the U1 has no cutter and forms no discrete tip;
  ...
  (`ams_backend_snapmaker.cpp:262-266`).
- `tool_to_slot_map` is seeded with identity because that is the literal truth about
  ...
  spool (`ams_backend_snapmaker.cpp:292-308`). Its consumers are the ones that need
...
- A slot select IS a physical tool change: `do_select_slot()` forwards to
  ...
  (`select_slot_moves_toolhead() = true`, `ams_backend_snapmaker.cpp:597-607`).
...
(`include/printer_discovery.h:433-436`). Registration order matters: a real aftermarket
...
and a bare `toolchanger` object alone is not enough (`include/printer_discovery.h:571-599`).
...
(`ams_backend_snapmaker.cpp:1174-1245`, `1456-1529`).

### RFID (filament_detect.info)

`parse_rfid_info()` reads per-channel tag fields
(`ams_backend_snapmaker.cpp:1083-1138`):
...
SUB_TYPE (`ams_backend_snapmaker.cpp:39-49`, `ams_backend_snapmaker.cpp:950-959`).

Every row above is code-verified against `parse_rfid_info()` and the apply loop
(`ams_backend_snapmaker.cpp:1083-1138`, `1171-1204`): tag identity rides
...
|----------------|----------|
| `AUTO_FEEDING EXTRUDER=<n> LOAD=1` | Load (`do_load_filament`, `ams_backend_snapmaker.cpp:458-484`) |
...
trail (`ams_backend_snapmaker.cpp:474-483`): bare `T{n}` is a no-op when the target tool
...
- **The operation step bar.** LOAD/manual/preload share a 5-step model
  ...
  (`ams_backend_snapmaker.cpp:360-399`).
- **The per-tool "loaded at toolhead" latch.** Set on `load_finish`; cleared on
  ...
  current firmware (`include/ams_backend_snapmaker.h:503-519`,
  `ams_backend_snapmaker.cpp:522-548`).
...
- **`preload_finish` is terminal-for-latch but does not end the operation** — a re-unload
  ...
  killed the unload step display mid-heat (`ams_backend_snapmaker.cpp:1568-1583`).
...
`ams_backend_snapmaker.cpp:1531-1545`). Lanes reaching `unload_finish` are reported to
...
(`ams_backend_snapmaker.cpp:1558-1567`, `ams_backend_snapmaker.cpp:1964-1969`) — the deferral exists because
calling into `AmsState` under our mutex inverted `add_backend()`'s lock order (TSan,
2026-08-16).

### Runout and Resume

`prepare_for_resume()` (`ams_backend_snapmaker.cpp:665-803`) classifies the pause first:
...
(`ams_backend_snapmaker.cpp:730-743`, `:722-724`). The full chain - runout pause ->
...
idle lane going empty needs no operator action) (`include/ams_backend_snapmaker.h:257-263`).
...
(`ams_backend_snapmaker.cpp:637-661`). The active tool's port-present flag it builds on
is still published to `AmsState::set_active_tool_port_present()` on change (#991), which
is what gates Resume in the runout dialog (`ams_backend_snapmaker.cpp:1895-1916`).
...
empty head and cancels the print on runout (`include/ams_backend_snapmaker.h:293-300`,
...
the firmware's default map (`ams_backend_snapmaker.cpp:2173-2222`). Full command
...
(`ams_backend_snapmaker.cpp:317-338`). Every parse tail runs the shared convergence:
...
(`ams_backend_snapmaker.cpp:1846-1893`, `ams_backend_snapmaker.cpp:1980-2023`).
...
hardware swap (`include/ams_backend_snapmaker.h:546-562`). Clears preserve
...
identity) (`ams_backend_snapmaker.cpp:2076-2112`).
...
(`ams_backend_snapmaker.cpp:934-1014`).
...
| Bypass | No | `supports_bypass = false`; both entry points `not_supported` — no external spool on a toolchanger (`ams_backend_snapmaker.cpp:260`, `ams_backend_snapmaker.cpp:1032-1038`) |
| Dryer | No | Not supported |
| Recover / Reset / Cancel | No | All three return `not_supported` (`ams_backend_snapmaker.cpp:621-631`) |
| Operation step bar | Yes | Firmware-driven per-direction steps via `ams_operation_phase`; Heat step live |
| Per-slot loaded authority | Override | `slot_is_actively_loaded()` returns `status == LOADED` verbatim (hub table, `ams_backend_snapmaker.cpp:588-595`) |
| Path visualization | Yes | NOZZLE when the latch is set, OUTPUT when port/motion sensor still sees filament, NONE otherwise (`ams_backend_snapmaker.cpp:416-448`) |
...
2. `is_stuck_motion_sensor_runout()` has no caller — revive when a verifiable
   "filament at the gear" signal exists (`ams_backend_snapmaker.cpp:637-661`). Checked
   ...
   (`ams_backend_snapmaker.cpp:149-157`, `:569-576`). What is missing is rig
...
3. End-to-end timing of the pre-print `SET_PRINT_USED_EXTRUDERS` is unverified live.
   ...
   UNCERTAIN".
4. The `prepare_for_resume` doc comment in `include/ams_backend_snapmaker.h:233-240`
