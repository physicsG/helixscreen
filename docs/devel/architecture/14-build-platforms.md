...
| `mk/cross.mk` | The platform matrix: per-`PLATFORM_TARGET` toolchain, flags, display backend, font tiers, feature defines, plus Docker and `deploy-*` targets |
| `mk/rules.mk` | The `all` target: two-phase `-j` auto-fix, architecture-change auto-clean, every compile/link rule |
| `mk/tests.mk` | `helix-tests` build and the Catch2 sharded parallel runner |
| `mk/patches.mk` | Submodule patch stamp, both-directions wiring check, `reset-patches` / `reapply-patches` |
| `mk/display-lib.mk` | Which display backends compile per OS; builds `libhelix-display.a` shared by splash and app |
| `patches/README.md` | Per-patch purpose table, upstream PR status, patch-regeneration recipes |
| `scripts/setup-worktree.sh` | Worktree one-shot: creates `.worktrees/<branch>`, symlinks `lib/`, configures ccache |
| `src/api/display_backend.cpp` | `DisplayBackend` factory and the runtime DRM→fbdev→SDL auto-detect |
...
configuration, per-patch regeneration — live in `BUILD_SYSTEM.md` and `patches/README.md`. What stays
...
target, `mk/rules.mk:123`) builds **only** `helix-screen`: patches, generated fonts, translations,
splash, watchdog, the binary, stripping, and the optional Bluetooth plugin. `make test`
(`mk/tests.mk:393`) builds **only** `helix-tests`; `make test-run` (`mk/tests.mk:400`) builds it and
...
(`mk/fonts.mk:179`), `make translations` / `make translation-sync` for the YAML string pipeline
(`mk/translations.mk:84`). Hand-editing a generated artifact is wasted work; the next build
regenerates it.

Two guards in `mk/rules.mk` make bare `make` safe. A two-phase `all` (`mk/rules.mk:78`) re-invokes
...
Every target below is one `ifeq` block in `mk/cross.mk` that sets a toolchain triple, `TARGET_CFLAGS`
...
| `pi`, `pi-fbdev`, `pi-both` | Raspberry Pi aarch64 | DRM+GLES / fbdev | `-both` compiles once, links DRM + fbdev (`mk/pi-dual-link.mk`) |
...
| `ad5m`, `ad5m-br` | FlashForge AD5M (armv7) | fbdev | Fully static; ships as a ready-made firmware image (Forge-X fork); `-br` is the buildroot variant built inside AD5M Klipper Mod (`AD5M_KMOD_VARIANT.md`) |
...
QIDI, Snapmaker U1) plus `AD5M_KMOD_VARIANT.md`; QIDI on-device (Q2, Max 4) uses the plain `pi`
...
(`mk/display-lib.mk:23`: fbdev+DRM always on Linux, SDL when `ENABLE_SDL`) but runtime *selection*:
`DisplayBackend::create_auto()` probes DRM, then fbdev, then SDL (`src/api/display_backend.cpp:242`),
...
submitted upstream, tracked in `patches/README.md`). `mk/patches.mk` applies them through a stamp
...
(`mk/patches.mk:195`): a `patches/*.patch` with no apply block in `mk/patches.mk`, or an apply block
...
- **Generated assets regenerate; don't hand-edit them.** New icons mean `include/ui_icon_codepoints.h` plus `make regen-fonts` plus a rebuild; user-facing strings flow through `make translation-sync` / `make translations`. If a font or translation "won't update", you are probably editing the generated file.
...
- **Feature availability differs per target by design.** Before debugging "missing" sound, label printing, or a dev panel on a device build, check the target's gates in `mk/cross.mk` — `HELIX_HAS_SOUND`, `HELIX_HAS_LABEL_PRINTER`, `ENABLE_DEV_PANELS` and friends are compile-time, not runtime settings.
- **Embedded-only code paths are invisible on the desktop build.** `LV_USE_EVDEV` is 1 only when the display backend is fbdev or DRM (`lv_conf.h:1152`), so an evdev (or other embedded-driver) change is never exercised by `native` — a non-SDL build such as `PLATFORM_TARGET=x86-fbdev` is the cheapest way to exercise it on a dev box.

## Going deeper

- `../BUILD_SYSTEM.md` — the reference manual: full target specifications, Docker image architecture, deploy and logging-on-target recipes, worktree and ccache detail, GCC 7.5 compatibility notes.
- `patches/README.md` — every patch's purpose and files, upstream PR status (a patch is droppable only when its PR is *merged*), and the pristine-file method for regenerating a patch whose file is shared.
...
2. `mk/rules.mk:78` — the two-phase `all` target: unlimited-`-j` detection and re-invocation; then `:123` for what a build actually gates on (`apply-patches` first).
3. `mk/rules.mk:49` — the `.build-target` arch-change marker and auto-clean.
4. `mk/tests.mk:393` — the `test` (build-only) vs `test-run` (parallel shards) split, and the `~[.] ~[slow]` filter convention.
5. `mk/cross.mk:8` — the commented platform menu; then `:58` (pi: DRM+GLES, all font tiers) against `:216` (ad5m: `-Os -flto -static`, label-printer gate off, trimmed fonts) to see how far the knobs turn.
...
13. `scripts/setup-worktree.sh:1` — the worktree one-shot: symlink strategy, ccache setup, and the `--unlink`/`--relink` options.
